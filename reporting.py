...
def report_training(
    ...
    print_section(f"1 plot saved to {PLOT_DIRS['training']}/")
    return r2_scores
...
def report_exploration_round(
    ...
    print_explore_row(code, params, perf, uncertainty, agent.last_opt_score, w_explore)
...
def report_exploration_summary(
    ...
    print_section(f"{n_explore + 1} plots saved to {PLOT_DIRS['exploration']}/")
...
def report_inference_summary(
    ...
    print_section(f"{n_infer + 4} plots saved to {PLOT_DIRS['inference']}/")
...
def report_adaptation(
    ...
    print_section(f"1 plot saved to {PLOT_DIRS['adaptation']}/")
...
def report_summary(
    ...
    print_run_summary(
        state.perf_history, state.all_phases, state.all_codes, design_intent,
