...
| `prediction_model.py` | `DevMLP` | `pred_fab.models.MLPModel` | PyTorch MLP (48,24,12) for path_deviation |
...
- MLPs subclass `pred_fab.models.MLPModel` — the framework owns the `nn.Module` + Adam/MSE training loop. Subclasses declare only `HIDDEN` and the IPredictionModel properties.
