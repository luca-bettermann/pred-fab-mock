...
```mermaid
graph TD
  SEN[sensors/] -->|cached readings + physics| FEAT[feature_models.py]
  FEAT -->|feature values| PF[pred_fab pipeline]
  PF -->|features to score| EVAL[evaluation_models.py]
  PF -->|training data| PRED[prediction_model.py]
  SC[schema.py] -->|print_speed bounds| EVAL
```
...
| `evaluation_models.py` | `ProductionRate` | `_LinearTargetScore` | `production_rate` vs MAX_RATE = `PRINT_SPEED_BOUNDS[1]` (schema) |
...
- `ProductionRate.MAX_RATE` derives from the schema's `PRINT_SPEED_BOUNDS` (no slip, full speed) — never a local literal.
...
- Evaluation models implement the multi-feature `IEvaluationModel` contract (`input_features` list, `_score_row` per-row numpy, `_score_tensor` batched torch for gradient acquisition); the framework orchestrates row iteration, batching, and NaN handling. `_score_tensor` clamps itself — `compute_performance_tensor` does no clipping, so that in-model clamp is load-bearing.
