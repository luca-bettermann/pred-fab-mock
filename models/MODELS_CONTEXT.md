...
| `prediction_model.py` | `ProductionRatePredictionModel` | `IPredictionModel` | Deterministic: rate = print_speed / 60; no training needed |
...
- `ProductionRatePredictionModel` is deterministic (no ML): forward_pass denormalizes print_speed from [0,1] and divides by 60.
